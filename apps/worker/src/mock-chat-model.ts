...
export class MockChatModel extends BaseChatModel {
  ...
  async _generate(
    ...
    if (
      userContent.toLowerCase().includes("game") ||
      userContent.toLowerCase().includes("title")
    ) {
      mockResponse +=
        "I would normally fetch game data from the RAWG API here. ";
    } else if (userContent.toLowerCase().includes("genre")) {
      mockResponse +=
        "I would normally fetch genre information from the RAWG API here. ";
    } else if (userContent.toLowerCase().includes("analysis")) {
      mockResponse +=
        "I would normally perform data analysis using the available tools here. ";
    } else {
      mockResponse += "I would normally process your request using real AI here. ";
    }

    mockResponse +=
      "Replace with GPT-4 via ChatOpenAI when OPENAI_API_KEY is configured.";
